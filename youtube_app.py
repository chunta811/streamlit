...
DEVELOPER_KEY = "AIzaSyAIOnUo6g-Ka-CnyOhUwb7HxiDIv_PTbp8"
...
def search_videos(youtube, q='自動化', max_results=50):
...
def get_results(df_video, threshold=5000):
    ...
    df = pd.merge(left=df_videos, right=df_subscribers, on='channel_id')
...
df_videos = search_videos(youtube, q=query, max_results=50)
results = get_results(df_videos, threshold=threshold)
